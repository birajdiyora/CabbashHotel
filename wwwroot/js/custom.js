(function ($) {
  ...
  $(function () {
    ...
    // Parse existing dates from model values or use defaults
    const $checkInInput = $('input[name="CheckInDate"]');
    const $checkOutInput = $('input[name="CheckOutDate"]');
    
    let startDate = today;
    let endDate = checkOutDefault;
    
    const currentCheckIn = $checkInInput.val();
    const currentCheckOut = $checkOutInput.val();
    
    console.log('Current CheckIn:', currentCheckIn);
    console.log('Current CheckOut:', currentCheckOut);
    
    // Parse dates from URL parameters if available
    const urlParams = new URLSearchParams(window.location.search);
    const urlCheckIn = urlParams.get('CheckInDate');
    const urlCheckOut = urlParams.get('CheckOutDate');
    
    console.log('URL CheckIn:', urlCheckIn);
    console.log('URL CheckOut:', urlCheckOut);
    
    if (urlCheckIn && moment(urlCheckIn, 'YYYY-MM-DD', true).isValid()) {
      startDate = moment(urlCheckIn, 'YYYY-MM-DD');
      console.log('Using URL CheckIn date:', startDate.format('YYYY-MM-DD'));
    } else if (currentCheckIn && currentCheckIn !== "June 16 - June 20") {
      if (moment(currentCheckIn, 'YYYY-MM-DD', true).isValid()) {
        startDate = moment(currentCheckIn, 'YYYY-MM-DD');
        console.log('Using input CheckIn date:', startDate.format('YYYY-MM-DD'));
      }
    }
    
    if (urlCheckOut && moment(urlCheckOut, 'YYYY-MM-DD', true).isValid()) {
      endDate = moment(urlCheckOut, 'YYYY-MM-DD');
      console.log('Using URL CheckOut date:', endDate.format('YYYY-MM-DD'));
    } else if (currentCheckOut && currentCheckOut !== "June 16 - June 20") {
      if (moment(currentCheckOut, 'YYYY-MM-DD', true).isValid()) {
        endDate = moment(currentCheckOut, 'YYYY-MM-DD');
        console.log('Using input CheckOut date:', endDate.format('YYYY-MM-DD'));
      }
    }
    
    // Only adjust checkout date if it's actually before checkin (not same day)
    if (endDate.isBefore(startDate)) {
      console.log('EndDate is before StartDate, adjusting...');
    ...
    // Function to update both displays and inputs
    function updateDateDisplays(start, end) {
      console.log('Updating displays - Start:', start.format('YYYY-MM-DD'), 'End:', end.format('YYYY-MM-DD'));
      
      // Update input values with individual dates (not range strings)
      $checkInInput.val(start.format("YYYY-MM-DD"));
      $checkOutInput.val(end.format("YYYY-MM-DD"));
      
      console.log('Set CheckIn input to:', start.format("YYYY-MM-DD"));
      console.log('Set CheckOut input to:', end.format("YYYY-MM-DD"));
      
      // Update display elements - make sure we target the right elements
      const checkInDisplay = start.format("D MMMM");
      const checkOutDisplay = end.format("D MMMM");
      
      // Force update of display elements
      $(".hotel-selected-date-checkin").empty().html(`<h6>${checkInDisplay}</h6><span>Check-In</span>`);
      $(".hotel-selected-date-checkout").empty().html(`<h6>${checkOutDisplay}</h6><span>Check-Out</span>`);
      
      console.log('Updated display elements:', checkInDisplay, checkOutDisplay);
      console.log('Final CheckIn input value:', $checkInInput.val());
      console.log('Final CheckOut input value:', $checkOutInput.val());
    }
    ...
    // Initialize daterangepicker for both check-in and check-out inputs
    $("input.hotel-checkin").daterangepicker(
      {
        opens: "center",
        startDate: startDate,
        endDate: endDate,
      ...
      function (start, end) {
        console.log('Date range selected - Start:', start.format('YYYY-MM-DD'), 'End:', end.format('YYYY-MM-DD'));
        // Update both inputs and displays when ANY date field is clicked
        updateDateDisplays(start, end);
      }
    );
    
    // Handle Apply button click to ensure proper form submission
    $("input.hotel-checkin").on('apply.daterangepicker', function(ev, picker) {
      console.log('Apply clicked - Start:', picker.startDate.format('YYYY-MM-DD'), 'End:', picker.endDate.format('YYYY-MM-DD'));
      updateDateDisplays(picker.startDate, picker.endDate);
    });
    
    // Handle Cancel button click
    $("input.hotel-checkin").on('cancel.daterangepicker', function(ev, picker) {
      console.log('Cancel clicked - reverting to original dates');
      // Keep the original dates
    });
    
    // Wait for DOM to be ready, then force update displays
    setTimeout(function() {
      console.log('Force updating displays with:', startDate.format('YYYY-MM-DD'), endDate.format('YYYY-MM-DD'));
      updateDateDisplays(startDate, endDate);
      
      // Also update the input values to match
      $checkInInput.val(startDate.format("YYYY-MM-DD"));
      $checkOutInput.val(endDate.format("YYYY-MM-DD"));
    }, 100);
  ...
  function updateGuestSummary() {
    ...
    // Support both Adults/adults and Children/children input names
    $('input[name="adults"], input[name="Adults"]').each(function () {
      totalAdults += parseInt($(this).val(), 10) || 0;
    });

    $('input[name="children"], input[name="Children"]').each(function () {
    ...
    // NOTE: Room count is intentionally NOT updated here.
    // Room count is managed by updateRoomSummary() when rooms are added/removed,
    // and by the server-rendered @Model.Rooms value on page load.
  }
  ...
  function updateRoomSummary() {
    ...
    $("#room-qty").text(roomCount);
    
    // Update hidden rooms inputs on both Home and Hotel pages
    $("#rooms-hidden").val(roomCount);
  ...
  function createNewRoom() {
    ...
    // Reset values - support both naming conventions
    $newRoom.find('input[name="adults"], input[name="Adults"]').val(1);
    $newRoom.find('input[name="children"], input[name="Children"]').val(0);
